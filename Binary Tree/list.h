...
namespace custom {
   ...
   *******************************************/
   template <class T>
   void list <T> ::sort() throw(const char*)
   {
      Node *head = NULL;   // new list head (empty list)
      Node *pCurrent;      // auxiliary current pointer
      Node *pPrevious;     // auxiliary previous pointer

   // insert the first element into list
      head = insertNode(head, pHead->data);

      // insert each node from this list to a new one
      for (Node *pSrc = this->pHead->pNext; pSrc; pSrc = pSrc->pNext)
      {
         // initialize pointer to head of list
         pCurrent = pPrevious = head;

         // tranverse list to find where to put new node
         while (pCurrent && *(pSrc->data) >= *(pCurrent->data) )
         {
            pPrevious = pCurrent;
         ...
         if (*(pSrc->data) < *(pPrevious->data))
         {
            // insert current element into list
            pCurrent = insertNode(pPrevious, pSrc->data);

            // check if the new node will be the head
            if (*(pSrc->data) < *(head->data))
            {
               // update head
               head = pCurrent;
            }
         ...
         else
         {
            ...
            insertNode(pPrevious, pSrc->data, true);
         }
   ...
   *******************************************/
   ...
   typename list <T> ::iterator list <T> ::find(const T &element)
   {
      ...
      for (pCurrent = this->pHead; pCurrent; pCurrent = pCurrent->pNext)
      {
